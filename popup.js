(function(){
  ...
  $('start').addEventListener('click', async ()=>{
    ...
    const cfg = {
      emulatePointer: usePointer,
      swipeCount: Number($('swipeCount')?.value || 4),
      swipeDistance: Number($('swipeDistance')?.value || 300),
      swipeSteps: Number($('swipeSteps')?.value || 6),
      swipeStepDelay: Number($('swipeStepDelay')?.value || 120)
    };
    setStatus('Starting auto-scroll...');
    await sendToActive({ type: 'start_nearby_only', cfg });
  });
  ...
  $('start-click').addEventListener('click', async ()=>{
    ...
    await sendToActive({ type: 'start_nearby_click_and_scrape', cfg });
  });
  ...
  $('export').addEventListener('click', async ()=>{
    ...
    chrome.storage.local.get({ nearby_only_profiles: [] }, async (res)=>{
      ...
      for(const p of arr){
        ...
        const filename = `${finalBase}.jpg`;
        const blob = await fetchAsBlob(url);
          if(blob){
            try{
              setStatus(`Processing image ${idx}/${arr.length}...`);
              const filter = document.getElementById('imgFilter')?.value || 'none';
              const processed = await processImageBlob(blob, filter, 'image/jpeg', 0.9);
              files.push({name: filename, data: processed});
            }catch(e){
              console.warn('processing failed, adding original', e);
              // attempt to convert original to jpeg as a fallback
              try{
                const converted = await processImageBlob(blob, 'none', 'image/jpeg', 0.9);
                files.push({name: filename, data: converted});
              }catch(_){
  ...
  async function exportAllProfileImagesZip(){
    chrome.storage.local.get({ nearby_only_profiles: [] }, async (res)=>{
      ...
      for(const p of arr){
        ...
        let imgs = Array.isArray(p.images) && p.images.length ? p.images.slice() : (p.image ? [p.image] : []);
        // limit to up to 4 images per profile (user requested 2-4 images when available)
        ...
        for(const u of imgs){
          ...
          if(blob){
            try{
              const filter = document.getElementById('imgFilter')?.value || 'none';
              const processed = await processImageBlob(blob, filter, 'image/jpeg', 0.9);
              files.push({name: path, data: processed});
            }catch(e){
              console.warn('processing failed for profile image, adding original', e);
              try{ const converted = await processImageBlob(blob, 'none', 'image/jpeg', 0.9); files.push({name: path, data: converted}); }catch(_){ files.push({name: path, data: blob}); }
            }
  ...
  // Apply selected filter (none|grayscale|pixelate|blur) to an image Blob and return a processed Blob.
  // By default the output is PNG, but callers can request JPEG via `outputMime` (e.g. 'image/jpeg').
  async function processImageBlob(blob, filter, outputMime = 'image/png', quality = 0.92){
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        try{
          ...
          canvas.toBlob((b)=>{ if(b) resolve(b); else reject(new Error('toBlob failed')); }, outputMime, quality);
